...
\\
A neural network consists of a number of layers of artificial neurons, called nodes.
In a fully connected network, each node is connected to every node in the next layer.
The connection strengths are called weights.
An image of a person can be fed into the network by setting the activation values of the first layer of the network, the input layer, to the individual pixel values of the image.
This information is then fed forward through the layers of the network until the output layer is reached.
In our example, this layer should consist of only two nodes.
...
\\
One of the main strengths of neural networks is their ability to generalize~\cite{gonfalonieri_understand_2020}.
